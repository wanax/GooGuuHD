...
-(void)getDailyStockNews{
    [Utiles getNetInfoWithPath:@"DailyStock" andParams:nil besidesBlock:^(id obj){
        ...
        [self.indicator.comIconView setImageWithURL:[NSURL URLWithString:self.imageUrl] placeholderImage:[UIImage imageNamed:@"defaultIcon"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
            if (!image){
                UILabel *label = [[[UILabel alloc] initWithFrame:CGRectMake(17,8,150,75)] autorelease];
                [label setBackgroundColor:[UIColor whiteColor]];
                [label setText:obj[@"companyname"]];
                label.textAlignment = NSTextAlignmentCenter;
                label.layer.cornerRadius=3.0;
                [self.indicator addSubview:label];
            }
        }];
