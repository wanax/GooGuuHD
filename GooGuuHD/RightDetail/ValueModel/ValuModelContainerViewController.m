...
- (void)viewDidLoad
{
    ...
    if (![GetConfigure(@"userconfigure", @"showTip", YES) isEqualToString:@"1"]) {
        TipView *t=[[[TipView alloc] initWithFrame:CGRectMake(0,0,924,SCREEN_HEIGHT)] autorelease];
        self.tip=t;
        
        UIPanGestureRecognizer *tap=[[[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(tipViewIsClicked:)] autorelease];
        [self.tip addGestureRecognizer:tap];
        
        [self.view addSubview:self.tip];
        SetConfigure(@"userconfigure", @"showTip", @"1");
    }
}

-(void)tipViewIsClicked:(UIPanGestureRecognizer *)tap{
