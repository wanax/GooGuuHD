...
-(void)viewDidAppear:(BOOL)animated{
    ...
    if ([GetConfigure(@"userconfigure", @"agreement", YES) equals:@"1"]) {
        [self.checkHookImg setImage:[UIImage imageNamed:@"hookImg"]];
    }
...
-(void)initComponents{
    ...
    self.checkHookImg.userInteractionEnabled = YES;
    UITapGestureRecognizer *imgTap = [[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(checkHookImgClicked)] autorelease];
    [self.checkHookImg addGestureRecognizer:imgTap];
...
-(IBAction)agreementBtClicked:(id)sender {
    AgreementViewController *agreement = [[[AgreementViewController alloc] init] autorelease];
    agreement.modalPresentationStyle = UIModalPresentationFormSheet;
    [self presentViewController:agreement animated:YES completion:nil];
}

-(void)checkHookImgClicked {
    if ([GetConfigure(@"userconfigure", @"agreement", YES) equals:@"1"]) {
        [self.checkHookImg setImage:nil];
        SetConfigure(@"userconfigure", @"agreement", @"0");
    } else {
        [self.checkHookImg setImage:[UIImage imageNamed:@"hookImg"]];
        SetConfigure(@"userconfigure", @"agreement", @"1");
    }
}
...
-(IBAction)loginBtClicked:(id)sender{
    if ([GetConfigure(@"userconfigure", @"agreement", YES) equals:@"1"]) {
        ...
        [Utiles showToastView:self.view withTitle:nil andContent:@"您尚未同意用户协议" duration:1.0];
    }
}
